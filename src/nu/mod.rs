...
impl StreamingPlugin for Handler {
    fn signature(&self) -> Vec<PluginSignature> {
        ...
        vec![
            PluginSignature::build("from sse")
                .usage("Converts HTTP SSE (Server-Sent Events) into structured events")
                .search_terms(vec!["sse".to_string(), "stream".to_string(), "http".to_string()])
                ...
                .plugin_examples(vec![PluginExample {
                    ...
                    description: "Converts SSE from an HTTP GET request into structured events".to_string(),
